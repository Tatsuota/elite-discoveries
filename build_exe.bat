...
REM Output: dist\Elite Discoveries.exe  (double-click to run; no Python needed)
...
".venv\Scripts\python.exe" -m PyInstaller --noconfirm --onefile --windowed ^
    ...
    --icon "elite-discoveries.ico" ^
    --add-data "web;web" ^
    desktop.py || goto :error

echo.
echo Done. Your standalone app is: dist\Elite Discoveries.exe
